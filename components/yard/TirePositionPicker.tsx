...
export default function TirePositionPicker({ onConfirm, onBack }: Props) {
  ...
  return (
    ...
    >
      {/* Confirm back dialog */}
      {confirmBack && (
        <div className="bg-white rounded-2xl shadow-2xl flex flex-col items-center gap-4" style={{ padding: '28px 32px', marginBottom: '12px', minWidth: '280px' }}>
          <p className="font-bold text-slate-800 text-center" style={{ fontSize: '17px' }}>לצאת ללא בחירת צמיג?</p>
          <p className="text-slate-500 text-center text-sm">הצמיג לא יתווסף לכרטיס העבודה</p>
          <div className="flex gap-3 w-full">
            <button
              onClick={() => setConfirmBack(false)}
              className="flex-1 rounded-xl border-2 border-slate-300 font-bold text-slate-700 active:bg-slate-100 transition-colors"
              style={{ padding: '11px' }}
            >
              המשך בחירה
            </button>
            <button
              onClick={() => onBack?.()}
              className="flex-1 rounded-xl font-bold text-white active:opacity-80 transition-colors"
              style={{ padding: '11px', background: '#dc2626' }}
            >
              יציאה
            </button>
          </div>
      ...
      <div className="bg-white rounded-2xl flex flex-col items-center gap-5 shadow-2xl" style={{ padding: '28px 32px' }}>
        <div className="w-full flex items-center justify-between">
          {onBack && (
            <button
              ...
              className="rounded-xl border-2 border-slate-200 font-bold text-slate-600 active:bg-slate-100 transition-colors"
              style={{ padding: '6px 14px', fontSize: '14px' }}
            >
              ← חזרה
            </button>
          )}
          <p className="font-bold text-slate-800 flex-1 text-center" style={{ fontSize: '18px' }}>איזה גלגלים הוחלפו?</p>
          {onBack && <div style={{ width: '72px' }} />}
        </div>
        ...
        <button
          ...
          className="w-full rounded-xl text-white font-bold transition-colors disabled:opacity-30"
