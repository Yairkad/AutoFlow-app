...
export default function CustomersClient() {
  ...
  const [debts, setDebts]           = useState<CustomerLedgerDebt[]>([])
  ...
  const loadAll = useCallback(async () => {
    ...
    const [custRes, debtRes, catRes] = await Promise.all([
      supabase.from('customers').select('*').eq('tenant_id', tid).order('name'),
      supabase.from('customer_ledger_debts')
        .select('id,customer_id,amount,paid,description,date,is_closed,doc_type,doc_number,direction,invoices')
        .eq('tenant_id', tid),
    ...
    if (custRes.data) {
      ...
      if (!didAutoOpen.current) {
        ...
        if (openId) {
          const target = custRes.data.find(c => c.id === openId)
  ...
  return (
    <div>
      <PageHeader
        ...
        iconShadow="#78716c44"
        title="לקוחות (כרטסת)"
        subtitle="ניהול לקוחות הקפה, יתרות ותנועות"
      />
          ...
          <div className="supplier-detail-panel" style={{
            ...
            <div>
              ...
              <div style={{ marginTop: '14px', textAlign: 'center' }}>
                <a href={`/customer-tracking?open=${selected?.id ?? ''}`} style={{ fontSize: '12px', color: 'var(--primary)', textDecoration: 'none', fontWeight: 600 }}>
                  → מעקב לקוחות מפורט
                </a>
              </div>
            </div>
